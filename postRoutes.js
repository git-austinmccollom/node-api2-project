    ...
    data
      ...
      .then((dbRes) => {
        ...
        data
          ...
          .catch((findErr) => {
            res.status(500).json({ errorMessage: "server error adding to the database" });
          });
    ...
    if ( comment.text ) {
        ...
        .then( findRes => {
            if (findRes === []) {
                ...
                .then( commentRes => {
                    data.findCommentById( commentRes.id )
                    .then( findCommentRes => {
                        res.status(201).json(findCommentRes)
                    })
                    .catch( findCommentErr => {
                        res.status(500).json({ errorMessage: "could not find after creating"})
                    })
                })
            }
        })
